                ...
                {numbers.map((num) => {
                    ...
                    return (
                        <button
                            disabled={!inStock}
                            onClick={() => toggle(num)}
                            className={`py-2 px-4 lg:px-0 text-center rounded-md cursor-pointer transition hover:bg-zinc-400 disabled:bg-[#D2D1D3] disabled:text-[#8F8C91]  ${found ? "bg-black text-white" : "bg-white text-black"
                                }`}
                        >
                            {num}
                        </button>
